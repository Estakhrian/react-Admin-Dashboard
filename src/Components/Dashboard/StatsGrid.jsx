       ...
       dark:hover:shadow-slate-900/20 transition-all duration-200 group overflow-hidden'>
                    <div className='flex items-start justify-between'>
                        ...
                        <div className='flex-1'>
                            ...
                            <p className='text-lg sm:text-xl md:text-2xl font-bold text-slate-800 dark:text-white mb-4'>{stat.value}</p>
                            <div className='flex items-center space-x-1'>
                                {stat.trend === "up" ? <ArrowUpRight className='w-3 sm:w-4 h-3 sm:h-4 text-emerald-500' /> :
                                    <ArrowDownRight className='w-3 sm:w-4 h-3 sm:h-4 text-red-500' />}
                                <span className={`text-xs sm:text-sm font-semibold ${stat.trend === "up" ? "text-emerald-500" : "text-red-500"}`}>{stat.change}</span>
                                <span className='text-xs text-slate-500 dark:text-slate-400'>vs Last month</span>
                            </div>
                        </div>
                        <div className={`p-2 rounded-xl ${stat.bgColor} group-hover:scale-110 transition-all duration-300`}>
                            <stat.icon className={`w-3 sm:w-4 md:w-5 h-3 sm:h-4 md:h-5 ${stat.textColor}`} />
                        </div>
                    </div>
                    {/**progressbar */}
                    <div className='mt-4 h-1 sm:h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden'>
